...
export const listGalleryAdmin: RequestHandler = asyncHandler(async (req, res) => {
  const { page, pageSize, skip } = parsePagination(req.query as { page?: string; pageSize?: string });
  const [total, rows] = await Promise.all([
    GalleryItem.countDocuments({}),
    GalleryItem.find({}).sort({ sortOrder: 1, createdAt: 1 }).skip(skip).limit(pageSize).lean(),
  ]);
  res.json({
